...
from imusensor.MPU9250 import config2
...
class MPU9250:
	...
	def __init__(self, bus, address):
		...
		self.cfg2 = config2.getConfigVals()
		self.cfg2.Address = address
	...
	def begin(self):
		...
		self.__writeRegister(self.cfg2.PowerManagement1, self.cfg2.ClockPLL)
		self.__writeRegister(self.cfg2.UserControl, self.cfg2.I2CMasterEnable)
		self.__writeRegister(self.cfg2.I2CMasterControl, self.cfg2.I2CMasterClock)

		self.__writeAK8963Register(self.cfg2.Ak8963CNTL1, self.cfg2.Ak8963PowerDown)
		#self.__writeRegister(self.cfg2.PowerManagement1, self.cfg2.PowerReset) # power is not reseting
		time.sleep(0.01)
		#self.__writeAK8963Register(self.cfg2.Ak8963CNTL2, self.cfg2.Ak8963Reset) # AK8963 is not resetting
		self.__writeRegister(self.cfg2.PowerManagement1, self.cfg2.ClockPLL)

		name = self.__whoAmI()
		if not (name[0] == 113 or name[0] == 115 ):
			print ("The name is wrong {0}".format(name))
		self.__writeRegister(self.cfg2.PowerManagement2, self.cfg2.SensorEnable)
		...
		self.__writeRegister(self.cfg2.SMPDivider, 0x00)
		...
		self.__writeAK8963Register(self.cfg2.Ak8963CNTL1, self.cfg2.Ak8963FuseROM)
		time.sleep(0.1)
		self.__writeAK8963Register(self.cfg2.Ak8963CNTL1, self.cfg2.Ak8963ContinuosMeasurment2)
		time.sleep(0.1)
		self.MagScale = self.__readAK8963Registers(self.cfg2.Ak8963ASA, 3)
		...
		self.__writeRegister(self.cfg2.PowerManagement1, self.cfg2.ClockPLL)
		self.__readAK8963Registers(self.cfg2.Ak8963HXL, 7)
	...
	def setSRD(self, data):
		...
		self.__writeRegister(self.cfg2.SMPDivider, 19)

		if data > 9:
			self.__writeAK8963Register(self.cfg2.Ak8963CNTL1, self.cfg2.Ak8963PowerDown)
			time.sleep(0.1)
			self.__writeAK8963Register(self.cfg2.Ak8963CNTL1, self.cfg2.Ak8963ContinuosMeasurment1)
			time.sleep(0.1)
			self.__readAK8963Registers(self.cfg2.Ak8963HXL, 7)
		else:
			self.__writeAK8963Register(self.cfg2.Ak8963CNTL1, self.cfg2.Ak8963PowerDown)
		...
		self.__writeRegister(self.cfg2.SMPDivider, data)
	...
	def setAccelRange(self, accelRange):
		...
		try:
			self.__writeRegister(self.cfg2.AccelConfig, self.cfg2[accelRange])
		...
		self.AccelScale = self.cfg2.Gravity*accelVal/32767.5
		return 1
	...
	def setGyroRange(self, gyroRange):
		...
		try:
			self.__writeRegister(self.cfg2.GyroConfig, self.cfg2[gyroRange])
		...
		self.GyroScale = self.cfg2.Degree2Radian*(gyroVal/32767.5)
		return 1
	...
	def setLowPassFilterFrequency(self, frequency):
		...
		try:
			self.__writeRegister(self.cfg2.AccelConfig2, self.cfg2[frequency])
			self.__writeRegister(self.cfg2.GyroConfig2, self.cfg2[frequency])
	...
	def readRawSensor(self):
		...
		data = self.__readRegisters(self.cfg2.AccelOut, 20)

		data = np.array(data).astype(np.int16)
		highbits = data[::2]<<8
		vals = highbits + data[1::2]

		self.RawAccelVals = np.squeeze(self.cfg2.transformationMatrix.dot((vals[np.newaxis,:3].T)))*self.AccelScale
		self.RawGyroVals = np.squeeze(self.cfg2.transformationMatrix.dot((vals[np.newaxis,4:7].T)))*self.GyroScale
	...
	def readSensor(self):
		...
		data = self.__readRegisters(self.cfg2.AccelOut, 21)
		...
		self.AccelVals = (np.squeeze(self.cfg2.transformationMatrix.dot((vals[np.newaxis,:3].T)))*self.AccelScale - self.AccelBias)*self.Accels
		self.GyroVals = np.squeeze(self.cfg2.transformationMatrix.dot((vals[np.newaxis,4:7].T)))*self.GyroScale - self.GyroBias
		...
		self.Temp = (vals[3] - self.cfg2.TempOffset)/self.cfg2.TempScale + self.cfg2.TempOffset
	...
	def caliberateAccelerometer(self):
		...
		self.AccelBias = -1*self.cfg2.Gravity*self.AccelBias

		self.Accels[0] = (2.0*self.cfg2.Gravity)/(abs(xscale[0]) + abs(xscale[1]))
		self.Accels[1] = (2.0*self.cfg2.Gravity)/(abs(yscale[0]) + abs(yscale[1]))
		self.Accels[2] = (2.0*self.cfg2.Gravity)/(abs(zscale[0]) + abs(zscale[1]))
	...
	def __getScale(self, scale):
		if len(scale) == 0:
			...
			return sum(scale)/(2*self.cfg2.Gravity)
	...
	def __writeAK8963Register(self, subaddress, data):
		...
		self.__writeRegister(self.cfg2.I2CSlave0Address, self.cfg2.Ak8963I2CAddress)
		self.__writeRegister(self.cfg2.I2CSlave0Register, subaddress)
		self.__writeRegister(self.cfg2.I2CSlave0Do, data)
		self.__writeRegister(self.cfg2.I2CSlave0Control, self.cfg2.I2CSlave0Enable | 1)
	...
	def __readAK8963Registers(self, subaddress, count):
		...
		self.__writeRegister(self.cfg2.I2CSlave0Address, self.cfg2.Ak8963I2CAddress | self.cfg2.I2CReadFlad)
		self.__writeRegister(self.cfg2.I2CSlave0Register, subaddress)
		self.__writeRegister(self.cfg2.I2CSlave0Control, self.cfg2.I2CSlave0Enable | count)

		time.sleep(0.01)
		data = self.__readRegisters(self.cfg2.ExtSensData00, count)
		return data

	def __whoAmI(self):

		data = self.__readRegisters(self.cfg2.WhoAmI, 1)
		return data

	def __whoAmIAK8963(self):

		data = self.__readAK8963Registers(self.cfg2.Ak8963WhoAmI, 1)
		return data
